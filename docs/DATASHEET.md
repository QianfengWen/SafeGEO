...
of three randomly sampled attack targets, and the source corpus an agent would read.

Each base case is expanded into 68 instances:

- 22 attack packages applied to each of the 3 sampled targets (66 attacked instances), plus
...
- Every visible query is the construction query. Product and vendor names in the candidate
  roster and source packet use synthetic replacements. All decision-relevant hard constraints
  and soft preferences remain in the natural-language request. An exact audit over all 600
...
| `instances_manifest` | 40,800 | Manifest mapping each instance to base case, package, and attacked candidate. |
...
the mapping from sampled candidate IDs to controlled documents, the candidate-quality judgments needed
...
production shopping-research traces, then canonicalized. Product and vendor names are replaced
with synthetic names, and candidate/source records use synthetic IDs. The seller-source
...
- 600 base cases, with 18 to 22 candidates per query and 2 to 3 ground-truth candidates per
  query.
- Three eligible non-ground-truth targets per base case, sampled without replacement using a
  fixed release seed. The eligible pool contains lower-utility candidates outside the primary
  and acceptable benchmark-reference set. Candidate-specific constraint and evidence properties
  remain in the hidden reference annotations.
...
The released product/vendor names and record identifiers are synthetic, and the dataset contains
no intended personal or human-subject data.
...
- **Request alignment.** Every visible `user_query` is the construction query. The exact check
  over 600 cases and 40,800 expanded rows is implemented in
...
Because the released artifact uses synthetic product/vendor names and identifiers and is confined to consumer product recommendation, results
